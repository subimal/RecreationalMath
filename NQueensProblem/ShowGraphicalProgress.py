...
class Board:
    ...
    def __init__(self,N,Progress=True):
        ...
        '''
        #--------------------------------------------------------------------------#
        from GUI import init_board_im, draw_board
        self.size=N
        self.queen=[None]*N 
        self.freecols=[[]]*N #map(lambda x: [], range(N))
        self.sols=[]
        self.surface, self.ball, self.sparams = None, None, None
    #------------------------------------------------------------------------------#
    ...
    def PutQueen(self, qno):
	    ...
	    # remaining ones are listed as her freecols
            ...
            for each in self.queen:
                if each!=None: qat.append(each)
	    #draw_board(self.surface, self.ball, self.sparams, [])
            draw_board(self.surface, self.ball, self.sparams, [-1]*self.size, colors = [(255,200,200), (100,100,100)])
            draw_board(self.surface, self.ball, self.sparams, qat, colors = [(255,200,200), (100,100,100)])
            sleep(.5)
		...
		#sol= map(lambda x: self.queen[x], range(self.size))
		    ...
		    # not have this earlier.
                    ...
                    draw_board(self.surface, self.ball, self.sparams, qat, colors = [(100,255,100), (100,100,100)])
                    sleep(.5)
...
def SolveNQueens(size, ShowProgress=False):
    ...
    from GUI import init_board_im, draw_board
    from time import sleep
    board=Board(size, Progress=ShowProgress)
    if Progress: 
        [board.surface, board.ball, board.sparams] = init_board_im(board.size)
    board.PutQueen(0)
    if size==1:
    ...
    while board.freecols.count([])<len(board.freecols):
        ...
        if ShowProgress: 
            Progress(board)
            #if len(board.sols)>=1: 
	    #	draw_board(board.surface, board.ball, board.sparams, board.sols[-1], colors = [(200,255,200), (100,100,100)])
	    #   sleep (.1)
...
for N in range(1,12):
    sp=True
    b=SolveNQueens(N, ShowProgress=sp)
    if sp: pygame.quit()
    #PrintBoards(b)
